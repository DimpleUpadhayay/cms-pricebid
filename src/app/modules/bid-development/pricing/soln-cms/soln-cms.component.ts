...
@Component({
  selector: 'app-soln-cms',
  templateUrl: './soln-cms.component.html',
  styleUrls: ['./soln-cms.component.css'],
  providers: [AppModuleService, UsersService, PocDashboardService, ProjectScopeService]
})
export class SolnCmsComponent implements OnInit, OnDestroy {
  @ViewChild(AlertComponent) _alert: AlertComponent;
  user;
  attachment_data = [];
  userList = [];
  contributors = []; getdata
  temp = 0;
  productType = '';
  response = [];
  responseLength;
  ...
  constructor(private _bidService: BidService, private dialog: MatDialog,
    ...
    private _usersService: UsersService, private _sharedService: SharedService, private _psService: ProjectScopeService) {
    rootScope = this;
    ...
    this.mySubscription = this._sharedService.newData.subscribe(a => {
      if ((a.data == "pricing" || a.data == "review") && this.checkAction(this.response)) {
        // this.getReview();
        this.getPrePricingReview();
        this.readDataRefresh();
        this._sharedService.reviewType.emit({ type: 'pricing' });
      }
    });
    this.accessControl();
  ...
  getBidById() {
    ...
    this._bidService.getBidById(this.bid_id).subscribe(resp => {
      ...
      this.reviewers = this.bidData.participants.filter(a => {
        return a.userTypes[0].user_type == "REVIEWER" && (a.userTypes[0].user_subtype == "Pricing" || a.userTypes[0].user_subtype == 'Delivery' || a.userTypes[0].user_subtype == "Sales" || a.userTypes[0].user_subtype == 'Delivery' || a.userTypes[0].user_subtype == "Finance" || a.userTypes[0].user_subtype == "All")
      });
      this.loader = false;
    });
  }
  ...
  getReview() {
    ...
    this._bidService.getReviewData({ "bid_id": this.bid_id, "status": "ACTIVE" })
      .subscribe((resp) => {
        ...
        if ((this.reviewData[this.reviewData.length - 1].review_add.length > 0 && !this.reviewFlag)) {
  ...
  getPrePricingReview() {
    ...
    this._bidService.getNonPricingReviewData({ "bid_id": this.bid_id, "status": "ACTIVE" })
      .subscribe((resp) => {
        ...
        if (this.prePricingReviewData[this.prePricingReviewData.length - 1].review_add.length > 0 && !this.reviewNonPriceflag) {
          this.reviewComment = true;
        }
  ...
  checkAction(data) {
    ...
    if (data.length != 0) {
      let flag = false;
      if (data[this.temp].solution_add.length != 0) {
        data[this.temp].solution_add.forEach(element => {
      ...
      let contributor = this.response[this.temp].solution_add.filter(a => { return a.contributor == this.user.user_id; });
      if ((contributor == undefined || contributor.length == 0 && flag) || data[this.temp].solution_add[0].draft[0].flag) {
        validate = false;
  ...
  getContributorList() {
    ...
    console.log("Hello 257", this.contributors)
  }
  ...
  readData() {
    ...
    this._bidService.getSolutionData(obj).subscribe((resp: object) => {
      if (resp['data'] == null) {
        this.responseLength = 0;
        this.loader = false;
        return;
      ...
      for (let i = 0; i < respo.length; i++) {
        ...
        this.solutionCats.push(respo[i].solution_cat);
        this.solutionCatsIdName.push({ id: respo[i].solution_id, name: respo[i].solution_cat })
        this.responseLength = this.response.length;
      }
      this.selectedCategory = this.selectedCategory == '' ? this.solutionCats[0] : this.selectedCategory;
  ...
  onPricingAdd() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.responseLength++;
        this.solutionCats.push(result);
        this.disabledFlag = false;
        this.tempFlag = true;
        this._bidService.createSolutionData(obj).subscribe((resp) => {
          this.tempFlag = true;
          this.response = [];
          this.solutionCats = [];
          this.readDataRefresh();
  ...
  onDeleteCategory(item) {
    ...
    let data = this.response.filter(a => {
      return a.solution_cat == item;
    })
    this._alert.deleted("").then(success => {
      this._bidService.deleteSolutionCategory({ solution_id: data[0].solution_id, bid_id: this.bid_id }).subscribe(success => {
        this._bidService.refreshContent(this.refreshObj).subscribe(resp => {
        }, cancel => {
        });
        this.router.navigateByUrl('/RefrshComponent', { skipLocationChange: true }).then(() =>
          this.router.navigateByUrl('/bid-development/' + this.bid_id + '/pricing'));
      });
    }, cancel => {
      return;
    });
  ...
  onCloneCategory() {
    ...
    let data = {};
    data['categoryData'] = this.solutionCatsIdName;
    data['type'] = "pricing";
  ...
  addRow(index, temp) {
    this.disabledFlag = false;
    this.response[temp]['solution_add'].push({
      item_name: "",
      description: "",
      remarks: "",
      contributor: "",
    ...
    this.itemSoln.push(this.response[temp].solution_add[this.response[temp]['solution_add'].length - 1]);
  }

  // BM can delete the row in category
  deleteRow(index, temp) {
    if (this.response[temp].solution_add.length == 1 || this.pocSubmited || !this.response[this.temp].solution_add[0].draft[0].flag || (this.user_type == "BID_OWNER" && !this.isCoOwner) || this.bidStatus == 'DROPPED') {
      return;
    }
    ...
    if (dataval.solution_add[index].item_id == undefined) {
      this.response[temp]['solution_add'].splice(index, 1);
      return;
    }
    this._alert.deleted("").then(success => {
      this.loader = true
      let data = {
        // "solution_id": dataval.solution_id,
        "item_id": dataval.solution_add[index].item_id
      };
      this._bidService.deleteSolutionData(data).subscribe((resp) => {
        this.response[temp]['solution_add'].splice(index, 1);
        this.loader = false
      }, error => {
        this.loader = false
  ...
  onDateSelect(temp, index) {
    this.response[temp].solution_add[index].startDate = this.response[temp].solution_add[index].dateTimeRange[0];
    this.response[temp].solution_add[index].endDate = this.response[temp].solution_add[index].dateTimeRange[1];
  }
  ...
  onUpload(index, temp) {
    ...
    if (this.user_type != "BID_OWNER" && this.user_type != "CONTRIBUTOR") {
      ...
      return false;
    } else if (/* (this.user_type == "BID_OWNER" && !this.response[temp].solution_add[0].draft[0].flag) ||
      this.user_type == "CONTRIBUTOR" && */
      !this.tempFlag && this.response[temp].solution_add[index].contributor != this.user.user_id && !this.response[temp].solution_add[index].draft[0].flag) {
      return false;
    } else if (this.response[temp].solution_add[index].contributor == this.user.user_id &&
      this.response[temp].solution_add[index].draft.length > 1 &&
      !this.response[temp].solution_add[index].draft[1].flag) {
      return false;
    } else {
  ...
  onUploadReview(index, temp) {
    ...
    if (this.user_type != "BID_OWNER" && this.user_type != "CONTRIBUTOR") {
      this._alert.sweetError("Sorry, You are not authorised to upload documents")
      return false;
    } else if (this.response[temp].solution_add[index].subItem.length != 0 && !this.response[temp].solution_add[index].subItem[this.response[temp].solution_add[index].subItem.length - 1].draft[1].flag) {
      return false;
    }
    this.openDialog(index, temp)
  }
  ...
  openDialog(index, temp): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (this.response[this.temp].solution_add[index].subItem.length == 0) {
        let attach = this.response[temp].solution_add[index].attachment_data.concat(this.attachment_data);
        this.response[temp].solution_add[index].attachment_data = attach;
      }
      if (this.response[temp].solution_add[index].subItem.length != 0) {
        this.oldAttachments = [];
        if (this.response[temp].solution_add[index].subItem.length > 1)
          this.oldAttachments = this.response[temp].solution_add[index].subItem[this.response[temp].solution_add[index].subItem.length - 2].attachment_data;
        else
          this.oldAttachments = this.oldAttachments.concat(this.response[this.temp].solution_add[index].subItem[0].attachment_data);
        if (this.response[temp].solution_add[index].subItem[this.response[temp].solution_add[index].subItem.length - 1].attachment_data.length != 0) {
          this.attachment_data.forEach(element => {
            this.response[temp].solution_add[index].subItem[this.response[temp].solution_add[index].subItem.length - 1].attachment_data.push(element);
          });
        } else {
          this.response[temp].solution_add[index].subItem[this.response[temp].solution_add[index].subItem.length - 1].attachment_data = this.attachment_data;
        }
      }
  ...
  onDownloadDialog(index, temp) {
    let dt = this.response[temp].solution_add[index];
  ...
  onSelectCategory(item, i) {
    ...
    this.assignmentByBM = this.response[this.temp].solution_add[0].draft[0].flag;
    ...
    if (this.response.length != 0) {
      let contributor = [];
      this.response[this.temp].solution_add.forEach(element => {
      ...
      if ((contributor.length == 0 && this.user_type == 'CONTRIBUTOR' && flag) || this.response[this.temp].solution_add[0].draft[0].flag) {
        this.actionFlag = true;
  ...
  onSaveAsDraft(temp, type, upload) {
    this.response[temp].solution_add.forEach(element => {
    ...
    var type;
    if (type == "revoke") {
      type = "savePricingRevoke";
    } else if (type == 'save') {
      type = "saveSolutionData";
    }

    if (this.response[temp].date_created && this.user_type == "BID_OWNER" && this.response[temp].solution_add[0].draft.length == 1) {
      if (upload != 'upload')
        this.loader = true;
    ...
    } else if (this.response[temp].solution_add[0].draft.length == 2) {
      ...
      obj['solution_id'] = this.response[temp].solution_id;
      obj['solution_cat'] = this.response[temp].solution_cat;
      obj['solution_add'] = contributor;
  ...
  onSubmit(index, temp) {
    ...
    if (this.duplicateItems(this.response[temp].solution_add)) {
    ...
    this.response[temp]['user_type'] = this.user_type;
    this.response[temp]['user_subtype'] = this.user_subtype;
    if (this.response[temp].date_created && this.user_type == "BID_OWNER" && this.response[temp].solution_add[0].draft.length == 1) { // submit for BM
      this._alert.added("").then(success => {
        this.loader = true;
        this._bidService.updateSolutionAttachment(this.response[temp]).subscribe(resp => {
          this.tempFlag = false;
          // this.response = [];
          // this.solutionCats = [];
          // this.itemSoln = [];
    ...
    } else if (this.response[temp].solution_add[0].draft.length > 1) { // submit for contributor
      let contributor = [];
      this.response[temp].solution_add.forEach(element => {
        contributor.push(element);
        if (element.subItem && element.subItem.length != 0) {
      ...
      this._alert.added("").then(success => {
        this.loader = true
        this._bidService.updateSolutionAttachment(/* this.subItemflag ? reqObj :  */obj).subscribe(resp => {
          this.actionFlag = false;
          this.tempFlag = false;
          // this.itemSoln = [];
          // this.response = [];
          ...
          this._sharedService.reviewType.emit({ type: 'pricing' });
          this._bidService.refreshContent(this.refreshObj).subscribe(resp => {
            // console.log("soln submitted by contributor >>>>>");
          }, cancel => {
          });
        }, error => {
          this.loader = false
  ...
  validate(index, temp) {
    let validate = false;
    let data, tempData;
    tempData = this.response[temp].solution_add[index];
  ...
  onEdit(index, temp) {
    if (!this.validate(index, temp) || ((!this.reviewFlag || !this.reviewNonPriceflag) && this.reviewComment) || this.bidStatus == 'DROPPED') {
      return;
    }
    // if(!this.reviewNonPriceflag){
    //   this._alert.sweetError("Bid is already under Delivery Review");
    //   return
    // }
    // if(!this.reviewFlag){
    //   this._alert.sweetError("Bid is already under Pricing Review");
    //   return
    // }
    ...
    this.response[temp].solution_add = this.revokeData;
    this.response[temp].solution_add.forEach(element => {
      this.contributors.forEach(con => {
        if (con.user_id == element.contributor) {
          element['user_type'] = con.user_type;
    ...
    this.response[temp]['user_type'] = this.user_type;
    this.response[temp]['user_subtype'] = this.user_subtype;
    this._bidService.savePricingRevoke(this.response[temp]).subscribe(resp => {
      this.revokeFlag = false;
      this.readDataRefresh();
      this._sharedService.reviewType.emit({ type: 'pricing' });
      this._bidService.refreshContent(this.refreshObj).subscribe(resp => {
  ...
  onRevoke(index, temp) {
    ...
    this.response[temp]['user_type'] = this.user_type;
    this.response[temp]['user_subtype'] = this.user_subtype;
    this._bidService.submitPricingRevoke(this.response[temp]).subscribe(resp => {
  ...
  onReassign(index, temp) {
    if (!this.access.writeAccess || ((!this.reviewFlag || !this.reviewNonPriceflag) && this.reviewComment) || this.bidStatus == 'DROPPED') {
      return;
    }
    if(!this.reviewNonPriceflag){
      this._alert.sweetError("Bid is already under Delivery Review");
    ...
    let selectedRow = this.response[temp].solution_add[index];
  ...
  onReassignSubmit(index, temp) {
    this.response[temp]['user_type'] = this.access.participants[0].userTypes[0].user_type;
    this.response[temp]['user_subtype'] = this.access.participants[0].userTypes[0].user_subtype;
    this.response[temp]['reassign'] = true;
    let reqObj = Object.assign({}, this.response[temp]);
    let reassignData;
    if (reqObj["solution_add"][index].subItem.length) {
      let subItems = reqObj["solution_add"][index].subItem;
      reassignData = [subItems[subItems.length - 1]];
    } else {
      reassignData = [reqObj["solution_add"][index]];
    }
    reqObj["solution_add"] = JSON.parse(JSON.stringify(reassignData));
    if (reqObj["solution_add"][0].contributor == "") {
      this._alert.sweetError("Please select the Contributor")
      return;
    }
    reqObj["solution_add"].forEach(element => {
    ...
    if (this.response[temp].date_created && this.user_type == "BID_OWNER") {
      this._alert.added("").then(success => {
        ...
        this._bidService.updateSolutionAttachment(reqObj).subscribe(resp => {
          this.reassignFlag = false;
          this.actionFlag = false;
          this.readDataRefresh();
  ...
  categoryIcon() {
    let flag = false;
    if (this.reviewFlag && !this.pocSubmited && !this.bidData.parent && this.bidStatus != 'DROPPED') {
      flag = true;
    }
    return flag;
  }
  ...
  disableRemarks() {
    let flag = false;
    if ((!this.reviewFlag && this.reviewComment) || this.pocSubmited || this.bidData.parent || this.bidStatus == 'DROPPED') {
      flag = true;
    }
    return flag;
  }
  ...
  disableSubmitButtonBeforeRevoke() {
    let flag = false;
    if ((!this.reviewFlag && this.reviewComment) || this.pocSubmited || !this.access.writeAccess || !this.actionFlag || this.bidData.parent || this.bidStatus == 'DROPPED' || this.reassignFlag) {
      flag = true;
    }
    return flag;
  }
  ...
  disableSubmitButtonAfterRevoke() {
    let flag = false;
    if ((!this.reviewFlag && this.reviewComment) || this.pocSubmited || !this.access.writeAccess || this.bidData.parent || this.bidStatus == 'DROPPED' || this.reassignFlag) {
      flag = true;
    }
    return flag;
  }
