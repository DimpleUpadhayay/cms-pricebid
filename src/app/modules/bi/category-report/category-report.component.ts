...
export class CategoryReportComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    */
    zingchart.node_click = function (p) {
      ...
      let i = 1;
      rootScope.data.catgoryBidData[p.plotindex]['bids'].forEach(element => {
        let obj = {
          "Sr No": i,
      ...
      rootScope.gridData['filename'] = "Deals by Category";
  ...
  getResult() {
    this._bidService.getcategoryOpportunity(this.obj).subscribe(resp => {
      this.data = resp['data'];
      // console.log(this.data);
      this.totalBids = this.data.totalBidsCount;
      this.totalBidValue = this.data.totalBidsValue;
      this.loader = false;
      this.plotPieChart();
    }, err => {
      this.loader = false;
  ...
  plotPieChart() {
    originalConfig.series = rootScope.data.finalArray;
    // originalConfig.title.text=`Deals by Category`;
    this.render();
  }
  ...
  onReload() {
    this.router.navigateByUrl('/RefrshComponent', { skipLocationChange: true }).then(() =>
      this.router.navigateByUrl('analysis/categories'));
  }
